(() => {
  ...
  function renderModels(){
    ...
    $('systemModel').innerHTML='<option value="">No system model selected</option>'+models.filter(m=>m.enabled&&m.status==='ready'&&m.provider?.enabled).map(m=>`<option value="${m.id}">${escapeHtml(m.display_name)} — ${escapeHtml(m.provider.display_name)}</option>`).join('');
    $('systemModel').value=(selected?.enabled&&selected?.status==='ready'&&selected?.provider?.enabled)?systemModelId:'';
  ...
  $('clearCredential').onclick=async()=>{const id=$('providerSelect').value;if(!id)return;const p=providerById(id);if(!p?.credential_secret_id)return status('No credential is currently configured.','error');if(!confirm(`Disconnect the ${p.display_name} provider?`))return;try{const {error}=await client.rpc('sadeeq_clear_provider_credential',{p_provider_id:id});if(error)throw error;await load();status('Provider disconnected.','success');}catch(err){status(err?.message||'Could not disconnect provider.','error');}};
  ...
  $('saveSystem').onclick=async()=>{const id=$('systemModel').value;if(!id)return status('Select a valid enabled model first.','error');const model=models.find(x=>x.id===id);const provider=providerById(model?.provider_id);if(!model||!model.enabled||model.status!=='ready')return status('The selected model is not ready.','error');if(!provider?.enabled)return status('Enable the model provider first.','error');if(!provider.credential_secret_id)return status('Configure the provider credential first.','error');$('saveSystem').disabled=true;try{const {error}=await client.from('sadeeq_system_model').update({model_id:id}).eq('id',true);if(error)throw error;systemModelId=id;renderModels();status('System Model updated. All default-mode bots will inherit it.','success');}catch(err){status(err?.message||'Could not set System Model.','error');}finally{$('saveSystem').disabled=false;}};
  async function toggleModel(id){const m=models.find(x=>x.id===id);if(!m)return;const nextEnabled=!m.enabled;try{const {error}=await client.from('sadeeq_ai_models').update({enabled:nextEnabled,status:nextEnabled?'ready':'disabled'}).eq('id',id);if(error)throw error;if(systemModelId===id&&!nextEnabled){const {error:systemError}=await client.from('sadeeq_system_model').update({model_id:null}).eq('id',true);if(systemError)throw systemError;systemModelId='';}await load();status(`Model ${nextEnabled?'enabled':'disabled'}.`,'success');}catch(err){status(err?.message||'Could not update model.','error');}}
