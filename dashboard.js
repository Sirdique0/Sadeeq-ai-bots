(() => {
  ...
  function setLoader(hidden){ loader.classList.toggle('hidden',hidden); }
  ...
  function handleNav(item,event){ const label=item.dataset.nav; if(label==='Dashboard')return; if(label==='Sadeeq AI'||label==='AI Models'||label==='All Bots'){enterModule(item,event);return;} if(label==='Create Bot'){ if(event) event.preventDefault(); if(navigating)return; navigating=true; closeDrawer(); item.classList.add('active'); document.body.classList.add('transitioning'); transition.classList.add('active'); transition.setAttribute('aria-hidden','false'); const reduced=window.matchMedia?.('(prefers-reduced-motion: reduce)').matches; window.setTimeout(()=>{ window.location.href='./create-bot.html'; }, reduced ? 60 : 620); return; } closeDrawer(); showToast('Module coming soon',`${label} is reserved for its dedicated Sadeeq AI level.`); }
