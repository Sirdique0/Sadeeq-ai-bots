(() => {
  ...
  document.querySelectorAll('button[data-nav]').forEach(button => button.addEventListener('click', () => { if (button.dataset.nav === 'Web/API Tools') { closeDrawer(); window.location.href = './web-api-tools.html?v=4'; return; } closeDrawer(); showToast('Module coming soon', `${button.dataset.nav} is reserved for its dedicated Sadeeq AI level.`); }));
